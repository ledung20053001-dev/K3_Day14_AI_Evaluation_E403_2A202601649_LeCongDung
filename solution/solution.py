...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The score is the arithmetic mean of the three answer-side metrics.
        """
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # Python's sort is stable, so chunks with equal overlap retain their
    # original retriever order. Returning a new list keeps the input unchanged.
    return sorted(
        contexts,
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "object mapping every criterion to a score from 0.0 to 1.0.\n\n"
            f"Question:\n{question}\n\nAnswer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for index, item in enumerate(scores_batch):
            ...
            if not numeric_scores:
                continue
            item_average = sum(numeric_scores) / len(numeric_scores)
            all_scores.extend(numeric_scores)

            position = item.get("position", item.get("response_position"))
            is_first = item.get("is_first")
            if is_first is True or position in (0, 1, "first", "A"):
                first_scores.append(item_average)
            elif is_first is False or position is not None:
                later_scores.append(item_average)
            elif index % 2 == 0:
                first_scores.append(item_average)
            else:
                later_scores.append(item_average)
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        Computes averages per metric, compares them, and lists regressions.
        """
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = (
                ...
                else "Review the trace and add this case to the regression suite"
            )
            ...
            rows.append(
                f"| {safe(failure_id)} | {safe(failure_type)} | "
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": (
                "Add a grounding check that rejects or qualifies claims not "
                "supported by retrieved context"
            ),
            "irrelevant": (
                "Improve intent detection and add few-shot examples that answer "
                "the user's exact question"
            ),
            "incomplete": (
                "Retrieve broader evidence and require the generator to cover "
                "every condition, exception, and requested sub-question"
            ),
            "off_topic": (
                "Add intent-routing tests and constrain the prompt to the "
                "requested Student Services topic"
            ),
            "refusal": (
                "Calibrate safety guardrails with allowed in-scope examples to "
                "reduce unnecessary refusals"
            ),
            "unknown": (
                "Inspect answer and retrieval traces, assign a failure type, "
                "and add the case to the regression benchmark"
            ),
        }
        suggestions = [
        ...
        general_actions = [
            "Review the lowest-scoring traces and verify gold evidence before changing the pipeline",
            "Add failed and borderline cases to the regression suite with metric-specific thresholds",
            "Monitor retrieval and answer metrics separately after each prompt, model, or corpus change",
        ]
